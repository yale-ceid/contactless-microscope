...
int CounterX = 0; // max is 1000
int CounterY = 0; // max is 1050
int CounterZ = 0; // max is 4000

//Pedal Setup
const int pushButtonPin = 52;           // the number of the pushbutton pin
...
void setup()
{
  ...
  posZ0 = -diff + analogRead(posZPin); //move in the + direction
  negZ0 = -diff + analogRead(negZPin); //move in the - direction
}
...
//    long int posYDelay = (1000000 / (10 + posY0 - posYRead));
        long int posYDelay = (1000000 / (10 + mapper(posYRead)));
...
long int negYDelay = (1000000 / (10 + mapper(negYRead)));
...
//    long int negXDelay = (1000000 / (10 + posX0 - posXRead));
    long int negXDelay = (1000000 / (10 + mapper(posXRead)));
...
//    long int posXDelay = (1000000 / (10 + negX0 - negXRead));
        long int posXDelay = (1000000 / (10 + mapper(negXRead)));
...
//    long int posZDelay = (1000000 / (10 + posZ0 - posZRead));
    long int posZDelay = (1000000 / (10 + mapper(posZRead)));
...
//    long int negZDelay = (1000000 / (10 + negZ0 - negZRead));
     long int negZDelay = (1000000 / (10 + mapper(negZRead)));
...
int mapper(int prox){
  return map(prox,945,30,30,1000);
  }
