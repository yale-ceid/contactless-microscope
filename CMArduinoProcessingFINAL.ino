...
void loop()
{
  ...
  if ((PREV_X != CounterX)||(PREV_Y != CounterY)||(PREV_Z != CounterZ)){
...
//  Serial.println(negYRead);
  ...
  // Y-
  else if ((posYRead >= posY0) && (negYRead <= negY0) && (CounterY >= 179))
  {
    myStepperY.step(-1);
...
void moveXAxis()
{
  ...
  // X-
  else if ((negXRead <= negX0) && (posXRead >= posX0) && (CounterX <= 958))
  {
